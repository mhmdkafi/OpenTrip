...
export function Dashboard({section,tripId}:{section:string;tripId?:string}) {
 const {loading,error,notice}=useWorkspace();
...
function Settings(){
 ...
 return <div className="section-stack"><Panel title="Identitas ruang kerja"><div className="business-profile"><div className="profile-mark">R</div><div><h3>Rimbaloka Trip</h3><p>Open trip & outdoor experiences</p><span className="badge">Administrator · Akses semua fitur</span></div></div><div className="settings-facts"><div><small>Nama aplikasi</small><strong>TripDash</strong></div><div><small>Zona waktu</small><strong>Asia/Jakarta (WIB)</strong></div><div><small>Mata uang</small><strong>Rupiah Indonesia (IDR)</strong></div></div></Panel><Panel title="Integrasi Google Sheets"><div className="integration-card"><div className="integration-icon"><FileSpreadsheet size={29}/></div><div><h3>Google Spreadsheet</h3><p>Baca respons Google Form langsung dari spreadsheet Anda.</p></div><span className={`badge ${google?.connected?"":"gray"}`}>{google?.connected?"Terhubung":"Belum terhubung"}</span></div>{error&&<p role="alert">{error}</p>}{google&&!google.configured&&<p className="empty-note">Koneksi belum tersedia. Administrator perlu melengkapi konfigurasi Google.</p>}<div className="flex flex-wrap gap-3 mt-5">{google?.configured&&<a className="td-button" href="/api/google/connect"><Link2 size={15}/>{google.connected?"Hubungkan ulang":"Hubungkan Google"}</a>}{google?.connected&&<button className="td-secondary" disabled={busy} onClick={()=>void run(async()=>{await requestJson("/api/google/status",{},"DELETE");setGoogle({...google,connected:false});})}>Putuskan koneksi</button>}</div><p className="settings-hint"><ShieldCheck size={15}/> Hanya membaca data. Spreadsheet asli tetap utuh.</p></Panel></div>;
}
