...
export function TripDetail({ tripId }: { tripId: string }) {
  ...
  return <div className="section-stack"><Link className="text-link" href={`${basePath}/trips`}><ArrowLeft size={16}/> Trip Schedule</Link><header className="trip-detail-heading"><div><span className="detail-kicker">{trip.volume ? `Edisi ${trip.volume}` : "Detail perjalanan"}</span><h1>{trip.title}</h1><p>{trip.departureDate && `${new Date(`${trip.departureDate}T12:00:00`).toLocaleDateString("id-ID", { weekday: "long" })}, `}{dateLabel(trip.departureDate, true)} <span>·</span> {people.length} peserta{trip.location && ` · ${trip.location}`}</p></div><span className={`badge ${`status-${tripStatus(trip, state, todayWib())}`}`}>{statusLabel[tripStatus(trip, state, todayWib())]}</span></header>
    <div className="detail-actions"><button className="td-secondary" onClick={() => setEditing(true)}><Pencil size={15}/> Edit trip</button><button className="td-secondary" onClick={() => setImporting(true)}><FileSpreadsheet size={15}/> Impor spreadsheet</button><button className="td-button" disabled={busy || !people.length || missing} onClick={() => void print()}><FileDown size={15}/>{"Unduh PDF absensi"}</button><Link href={`${basePath}/finance?trip=${tripId}`} className="text-link">Keuangan trip</Link></div>
