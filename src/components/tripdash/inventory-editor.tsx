...
export function InventoryEditor({item,onClose}:{item?:Inventory;onClose:()=>void}) {
  ...
  return <Modal title={item?"Edit barang":"Tambah barang"} onClose={onClose} wide><div className="inventory-editor"><p className="editor-description">{item?"Perbarui foto, identitas, dan jumlah stok barang.":"Tambahkan perlengkapan agar stok dan penggunaannya dapat dipantau."}</p><Form onSave={async data=>{
    ...
    if(available<0)throw new Error("Stok total harus mencakup barang rusak dan yang sedang dipinjam.");
    if(!item&&total<1)throw new Error("Stok awal minimal 1 barang.");
    ...
    <div className="editor-columns">
      ...
      <section><h3>Cara pencatatan</h3><div className="inventory-mode-options">{[{id:"returnable",title:"Dikembalikan",description:"Dipinjam untuk trip, lalu kembali ke stok.",Icon:RotateCcw},{id:"consumable",title:"Habis pakai",description:"Stok berkurang setiap kali digunakan.",Icon:Package}].map(({id,title,description,Icon})=><label key={id} className={mode===id?"selected":""}><input type="radio" name="stock-mode" value={id} checked={mode===id} disabled={loaned>0&&id!=="returnable"} onChange={()=>setMode(id)}/><Icon size={18}/><span><strong>{title}</strong><small>{description}</small></span></label>)}</div>{loaned>0&&<p className="editor-help">{loaned} barang sedang dipinjam. Selesaikan pengembalian sebelum mengubah cara pencatatan.</p>}</section>
      <section><h3>Jumlah stok</h3><div className="form-grid"><label className="form-label">Stok total<div className="stock-stepper"><button type="button" className="td-secondary" aria-label="Kurangi stok total" disabled={total<=minTotal} onClick={()=>setTotal(total-1)}><Minus size={16}/></button><input className="td-input" aria-label="Jumlah stok total" type="number" value={total} min={minTotal} max={100000} required onChange={e=>setTotal(Number(e.target.value))}/><button type="button" className="td-secondary" aria-label="Tambah stok total" disabled={total>=100000} onClick={()=>setTotal(total+1)}><Plus size={16}/></button></div>{!item&&<small>Minimal 1 barang saat pertama dibuat.</small>}</label>{item&&<Field label="Barang rusak" type="number" value={damaged} min={0} max={total} onChange={e=>setDamaged(Number(e.target.value))}/>}</div><div className={available<0?"editor-stock-summary invalid":"editor-stock-summary"}><span>Stok tersedia</span><strong>{available} barang</strong><small>Total {total} − rusak {damaged} − dipinjam {loaned}</small></div>{consumable&&<Field label="Ingatkan jika stok ≤" name="threshold" type="number" min={0} max={100000} defaultValue={item?.reorderLevel??5} required/>}</section>
      {item&&<section><Field label="Alasan perubahan (opsional)" name="reason" placeholder="Contoh: tambah stok atau ganti foto barang" maxLength={200}/></section>}</div>
